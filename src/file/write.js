...
export default async function writeFile(file) {
    ...
    zip.generateAsync({ type: "nodebuffer", compression: "DEFLATE" }).then(async function callback(buffer) {
        if (!/^[a-z0-9-&() +'.,?;@%]{1,}$/i.test(originalname)) originalname = id;
        await knex("files").insert({ file_uuid: id, name: new Date().getTime() + "_" + originalname, type: mimetype });
        let arrayfuffer = [];
        for (let i = 0; i < buffer.length / 262144; i++)
            ...
            else arrayfuffer.push({ chunk: buffer.slice(262144 * i, buffer.length), file_uuid: id });
